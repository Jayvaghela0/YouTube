...
BACKEND_URL = "https://yt-downloader-e6db.onrender.com"  # ✅ Apna backend URL likho
...
def delete_after_delay(file_path, delay=300):
    """5 minutes ke baad file delete karne ka function"""
    time.sleep(delay)
    try:
...
def get_formats():
    """Yeh function sabhi available video qualities return karega"""
...
def start_download():
    """Yeh function selected quality ka video download karega"""
    url = request.args.get("url")
...
def download_video_task(video_url, format_id, video_id):
    """Yeh function background me video ko download karega"""
    try:
        ydl_opts = {
            "format": format_id,
